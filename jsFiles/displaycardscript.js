...
setCode = setCode.replace("?=set=", ''); // remove the ?
console.log(setCode);
...
function DisplayCards(response){
    ...
    for (var i = 0; i < Object.keys(response.cards).length - 1; i++){
        ...
        let cardData = `<div class = "col-md-3" onclick = "location.href='./chosencard.html?=set=${cardId}';">` + `<img class = "fullsize" src = "${cardImage}" />` + "</div>";
